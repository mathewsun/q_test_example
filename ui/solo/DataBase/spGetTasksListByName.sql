ALTER PROCEDURE  [dbo].[spGetTasksListByName]
...
BEGIN
	...
	where 
	  o.IsDone = 0
	  AND o.[Count] > (o.[ReadyCount]) -- Дополнительная проверка, что заказ еще не выполнен
	  AND (	(o.[OrderType] = 1
				AND X.[IsJoinGroup] = 1
				)
			OR (o.[OrderType] = 2
		...
		AND (o.OnlyMyAccountsWork = 0 OR o.[UserId] = X.[UserId])
	  and not exists ( select *
						  from OrderEvents oe 
						  where x.[Login] = oe.Account 
							and o.[Url] = oe.[Object] 
						) 
	order by o.LastUpdate
...
GRANT EXECUTE
    ON OBJECT::[dbo].spGetTasksListByName TO PUBLIC
    AS [dbo];